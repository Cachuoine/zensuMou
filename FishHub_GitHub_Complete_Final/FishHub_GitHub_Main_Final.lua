...
local Config = {
    ...
    BgMain = Color3.fromRGB(14, 15, 20),           
    BgCard = Color3.fromRGB(24, 25, 32),           
    BorderColor = Color3.fromRGB(70, 72, 86),     
    ShowDebug = true,
    UITransparency = 0.25,     
...
-- Architecture: keep UI/theme in this file; only Function implementations are remote modules.
-- Replace FeatureScripts.<name>.URL later without rebuilding the UI.
...
-- Section/divider lines: bright around the center and softly fade toward both ends.
-- This is visual-only and never changes any section/card background.
local function AddLineFade(lineObject)
    if not lineObject or not lineObject:IsA("GuiObject") then
        return
    end

    local old = lineObject:FindFirstChild("SectionLineFade")
    if old then
        old:Destroy()
    end

    local gradient = Instance.new("UIGradient")
    gradient.Name = "SectionLineFade"
    gradient.Parent = lineObject
    gradient.Rotation = 0
...
progressBackground.BackgroundTransparency = 0.05
...
-- Loading accent is independent from Config.ThemeColor.
-- When a Rainbow transition is loading, this is driven directly by HSV.
local loadingRainbowMode = false
local loadingRainbowHue = 0
local LOADING_RAINBOW_STEP = 0.008

local function UpdateLoadingTheme(overrideColor)
    local color = (typeof(overrideColor) == "Color3" and overrideColor) or Config.ThemeColor
...
local function StartLoadingAnimation()
    ...
    -- Dedicated loading-rainbow driver. It never reads Config.ThemeColor while active.
    task.spawn(function()
        while loadingAnimationRunning and token == loadingAnimationToken and loadingScreen.Parent do
            if loadingRainbowMode then
                loadingRainbowHue = (loadingRainbowHue + LOADING_RAINBOW_STEP * (Config.RainbowSpeedPercent / 100)) % 1
                UpdateLoadingTheme(Color3.fromHSV(loadingRainbowHue, 1, 1))
            end
            task.wait(0.03)
        end
    end)
...
local function PlayAdvancedThemeLoading(targetColor, loadingTitleText, loadingSubtitleText)
    ...
    local loadingTitleUpper = string.upper(tostring(loadingTitleText or ""))
    loadingRainbowMode = string.find(loadingTitleUpper, "RAINBOW", 1, true) ~= nil
    if loadingRainbowMode then
        loadingRainbowHue = rainbowHue
        UpdateLoadingTheme(Color3.fromHSV(loadingRainbowHue, 1, 1))
    else
        UpdateLoadingTheme(targetColor)
    end
    ...
    loadingText.Text = loadingSubtitleText or (loadingRainbowMode and "Loading Rainbow Accent" or "Applying Theme & Reloading UI")
    loadingStatus.Text = "INITIALIZING"
...
themeDividerContainer.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
    themeDividerContainer.BackgroundTransparency = 0.08
    themeDividerContainer.ClipsDescendants = true
...
colorInputCard.BackgroundTransparency = 0.05
...
rainbowToggleCard.BackgroundTransparency = 0.05
...
transDividerContainer.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
    transDividerContainer.BackgroundTransparency = 0.08
    transDividerContainer.ClipsDescendants = true
...
rainbowSpeedCard.BackgroundTransparency = 0.05
...
transparencyCard.BackgroundTransparency = 0.05
...
hotkeyDividerContainer.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
    hotkeyDividerContainer.BackgroundTransparency = 0.08
    hotkeyDividerContainer.ClipsDescendants = true
...
hotkeyCard.BackgroundTransparency = 0.05
...
debugToggleCard.BackgroundTransparency = 0.05
...
local function ApplyGlobalTheme(newColor)
    -- IMPORTANT: Theme/Rainbow is an accent system only.
    -- Section/card backgrounds stay dark and are never assigned the rainbow color.
...
task.spawn(function()
    while gui and gui.Parent do
        if isRainbowRunning then
            ...
            TweenService:Create(rainbowStatusDot, TweenInfo.new(0.5), {BackgroundTransparency = 0.05}):Play()
            task.wait(0.5)
...
task.spawn(function()
    while gui and gui.Parent do
        if Config.ShowDebug then
            ...
            TweenService:Create(debugStatusDot, TweenInfo.new(0.5), {BackgroundTransparency = 0.05}):Play()
            task.wait(0.5)
...
debugDividerBetween.BackgroundTransparency = 0.05
...
table.insert(allHubLines, debugDividerBetween)
    AddLineFade(debugDividerBetween)
...
joinDiscordBtn.Text = "💬 Join Server"
...
local function CreateTabContent(name)
    ...
    tabFrame.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
    ...
    if name == "Home" then
        -- Home-only backdrop. Function/Creative stay transparent.
        local homeBackdrop = Instance.new("Frame")
        homeBackdrop.Name = "HomeBackdrop"
        homeBackdrop.Parent = tabFrame
        homeBackdrop.Size = UDim2.new(1, 0, 1, 0)
        homeBackdrop.Position = UDim2.new(0, 0, 0, 0)
        homeBackdrop.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
        homeBackdrop.BackgroundTransparency = 0.08
        homeBackdrop.BorderSizePixel = 0
        homeBackdrop.ZIndex = 0
        homeBackdrop.Active = false
        ...
        local function CreateSectionDivider(text, order)
            ...
            -- The line runs behind the title. Only the title plate is black.
            local leftLine = Instance.new("Frame")
            leftLine.Name = "LeftSectionLine"
            leftLine.Parent = dividerContainer
            leftLine.Size = UDim2.new(0.5, -62, 0, 2)
            ...
            AddLineFade(leftLine)

            local rightLine = Instance.new("Frame")
            rightLine.Name = "RightSectionLine"
            rightLine.Parent = dividerContainer
            rightLine.Size = UDim2.new(0.5, -62, 0, 2)
            rightLine.Position = UDim2.new(0.5, 62, 0.5, -1)
            ...
            AddLineFade(rightLine)

            local titlePlate = Instance.new("Frame")
            titlePlate.Name = "SectionTitlePlate"
            titlePlate.Parent = dividerContainer
            titlePlate.Size = UDim2.new(0, 0, 0, 24)
            titlePlate.Position = UDim2.new(0.5, 0, 0.5, 0)
            titlePlate.AnchorPoint = Vector2.new(0.5, 0.5)
            titlePlate.AutomaticSize = Enum.AutomaticSize.X
            titlePlate.BackgroundColor3 = Color3.fromRGB(5, 6, 9)
            titlePlate.BackgroundTransparency = 0
            titlePlate.BorderSizePixel = 0
            titlePlate.ZIndex = 4

            local titlePadding = Instance.new("UIPadding")
            titlePadding.Parent = titlePlate
            titlePadding.PaddingLeft = UDim.new(0, 12)
            titlePadding.PaddingRight = UDim.new(0, 12)

            local textLbl = Instance.new("TextLabel")
            textLbl.Parent = titlePlate
    ...
    AddLineFade(infoDivider)
        ...
        tabFrame.CanvasSize = UDim2.new(0, 0, 0, 410)
    elseif name == "Function" then
        -- Function architecture: one card = one remote feature module.
        -- UI/theme stay local; only the implementation is loaded from the URL.
        -- No separate UI/Theme/Core modules are required for this build.
        ...
        searchBarCard.BackgroundTransparency = 0.05
        ...
        subSearchBarCard.BackgroundTransparency = 0.05
        ...
        -- FishHub owns the Function Search + Back controls.
        -- Remote feature modules only provide their feature content/logic.
        local FeatureAPI = {
            ShowNotification = ShowNotification,
            SetContent = function(title, content)
                subPageTitle.Text = tostring(title or "")
                subPageContentText.Text = tostring(content or "")
            end,
            GetContent = function()
                return subPageTitle.Text, subPageContentText.Text
            end,
            GetConfig = function()
                return Config
            end,
        ...
        -- One registry entry per Function card. The UI stays in this single file;
        -- feature logic can be filled into each Action without splitting UI/Theme/Core.
        ----------------------------------------------------------------------
        -- FUNCTION MODULE REGISTRY
        -- Paste a raw GitHub URL into each URL field later.
        -- UI/theme stay in this file; only the feature implementation is remote.
        ----------------------------------------------------------------------
        local FeatureScripts = {
            Shop = {
                Icon = "🛒",
                Desc = "Item shop, tools, etc.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/Shop.lua", -- Paste raw GitHub URL here
            },
            ["Setting farm"] = {
                Icon = "⚙️",
                Desc = "Farm settings and combat configuration.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/SettingFarm.lua", -- Paste raw GitHub URL here
            },
            Farm = {
                Icon = "⚔️",
                Desc = "Auto farm level, bone, etc.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/Farm.lua", -- Paste raw GitHub URL here
            },
            ["Item and Quest"] = {
                Icon = "📦",
                Desc = "Items, quests and progression.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/ItemAndQuest.lua", -- Paste raw GitHub URL here
            },
            ["Teleport Island"] = {
                Icon = "🏝️",
                Desc = "Quick teleport to islands.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/TeleportIsland.lua", -- Paste raw GitHub URL here
            },
            Fruit = {
                Icon = "🍎",
                Desc = "Fruit collection and storage.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/Fruit.lua", -- Paste raw GitHub URL here
            },
            ESP = {
                Icon = "👁️",
                Desc = "Player, item and chest ESP.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/ESP.lua", -- Paste raw GitHub URL here
            },
            Setting = {
                Icon = "🛠️",
                Desc = "Hub configuration settings.",
                URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules/Setting.lua", -- Paste raw GitHub URL here
            }
        }

        local LoadedFeatureModules = {}
        local LoadingFeatureModules = {}

        local function TrimString(value)
            return tostring(value or ""):gsub("^%s*(.-)%s*$", "%1")
        end

        local function LoadFeatureScript(featureName)
            local feature = FeatureScripts[featureName]
            if not feature then
                return false
            end

            local url = TrimString(feature.URL)
            if url == "" then
                return false
            end

            if LoadingFeatureModules[featureName] then
                return false
            end
            ...
            -- If the remote module returned a reusable function/table, reuse it.
            local cached = LoadedFeatureModules[featureName]
            if type(cached) == "function" then
                local ok, err = pcall(cached, FeatureAPI)
                if not ok then
                    return false
                end
                return true
            elseif type(cached) == "table" and type(cached.Start) == "function" then
                local ok, err = pcall(cached.Start, cached, FeatureAPI)
                if not ok then
                    return false
                end
                return true
            ...
            LoadingFeatureModules[featureName] = true

            local ok, result = pcall(function()
                local source = game:HttpGet(url)
                local chunk, compileError = loadstring(source)
                if not chunk then
                    error(compileError or "Unable to compile remote script.")
                end
                return chunk()
            end)

            LoadingFeatureModules[featureName] = nil

            if not ok then
                return false
            end

            -- Supported remote styles:
            -- 1) return function() ... end
            -- 2) return { Start = function(self) ... end }
            -- 3) execute directly and return nothing
            if type(result) == "function" then
                LoadedFeatureModules[featureName] = result
                local runOK, runError = pcall(result, FeatureAPI)
                if not runOK then
                    return false
                end
            elseif type(result) == "table" then
                ...
                if type(result.Start) == "function" then
                    local runOK, runError = pcall(result.Start, result, FeatureAPI)
                    if not runOK then
                        return false
                    end
                end
        ...
        local function CreateCategoryCard(title, icon, desc, order)
            ...
            card.MouseEnter:Connect(function()
                TweenService:Create(card, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    BackgroundColor3 = Color3.fromRGB(34, 35, 45),
            ...
            card.MouseButton1Click:Connect(function()
                local feature = FeatureScripts[title]
                local url = feature and TrimString(feature.URL) or ""

                -- IMPORTANT: no URL = no navigation and no fake content page.
                if url == "" then
                    return
                end

                local loaded = LoadFeatureScript(title)
                if not loaded then
                    return
                end

                -- Remote module owns the selected feature UI/content.
                subSearchBox.Text = ""
        ...
        for index, title in ipairs(featureOrder) do
            local feature = FeatureScripts[title]
            CreateCategoryCard(title, feature.Icon, feature.Desc, index)
        end
    ...
    elseif name == "Creative" then
        ...
        creativeLayout.Padding = UDim.new(0, 12)
        creativeLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

        local creativePadding = Instance.new("UIPadding")
        creativePadding.Parent = tabFrame
        creativePadding.PaddingTop = UDim.new(0, 10)
        creativePadding.PaddingBottom = UDim.new(0, 14)
        creativePadding.PaddingLeft = UDim.new(0, 4)
        creativePadding.PaddingRight = UDim.new(0, 4)

        -- Let the ScrollingFrame calculate its exact content height.
        -- This prevents the Discord card from being partially clipped at the top/bottom.
        ...
        paddingFrame.Size = UDim2.new(1, 0, 0, 1)
        paddingFrame.BackgroundTransparency = 1
        paddingFrame.LayoutOrder = 1

        -- Creative: "Join Server" means the FishHub Discord server.
        local joinCard = Instance.new("Frame")
        joinCard.Name = "DiscordJoinServerCard"
        joinCard.Parent = tabFrame
        joinCard.Size = UDim2.new(0.92, 0, 0, 96)
        joinCard.BackgroundColor3 = Color3.fromRGB(18, 19, 25)
        joinCard.BackgroundTransparency = 0.06
        joinCard.BorderSizePixel = 0
        joinCard.LayoutOrder = 2
        joinCard.ZIndex = 2
        Instance.new("UICorner", joinCard).CornerRadius = UDim.new(0, 12)

        local joinStroke = Instance.new("UIStroke")
        joinStroke.Parent = joinCard
        joinStroke.Color = Config.ThemeColor
        joinStroke.Thickness = 1.2
        joinStroke.Transparency = 0.15
        table.insert(allHubStrokes, joinStroke)
        AddHoverGlow(joinCard, joinStroke)

        local joinTitle = Instance.new("TextLabel")
        joinTitle.Parent = joinCard
        joinTitle.Size = UDim2.new(1, -116, 0, 24)
        joinTitle.Position = UDim2.new(0, 14, 0, 13)
        joinTitle.BackgroundTransparency = 1
        joinTitle.Font = Enum.Font.GothamBold
        joinTitle.TextSize = 15
        joinTitle.TextColor3 = Color3.fromRGB(245, 245, 250)
        joinTitle.Text = "💬  JOIN SERVER"
        joinTitle.TextXAlignment = Enum.TextXAlignment.Left
        joinTitle.ZIndex = 3

        local joinHint = Instance.new("TextLabel")
        joinHint.Parent = joinCard
        joinHint.Size = UDim2.new(1, -116, 0, 34)
        joinHint.Position = UDim2.new(0, 14, 0, 40)
        joinHint.BackgroundTransparency = 1
        joinHint.Font = Enum.Font.Gotham
        joinHint.TextSize = 11
        joinHint.TextColor3 = Color3.fromRGB(155, 160, 178)
        joinHint.Text = "Join the FishHub Discord server and get updates."
        joinHint.TextWrapped = true
        joinHint.TextXAlignment = Enum.TextXAlignment.Left
        joinHint.ZIndex = 3

        local discordJoinBtn = Instance.new("TextButton")
        discordJoinBtn.Parent = joinCard
        discordJoinBtn.Size = UDim2.new(0, 88, 0, 38)
        discordJoinBtn.Position = UDim2.new(1, -102, 0.5, -19)
        discordJoinBtn.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
        discordJoinBtn.BorderSizePixel = 0
        discordJoinBtn.AutoButtonColor = false
        discordJoinBtn.Font = Enum.Font.GothamBold
        discordJoinBtn.TextSize = 11
        discordJoinBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        discordJoinBtn.Text = "JOIN"
        discordJoinBtn.ZIndex = 3
        Instance.new("UICorner", discordJoinBtn).CornerRadius = UDim.new(0, 7)

        local discordJoinStroke = Instance.new("UIStroke")
        discordJoinStroke.Parent = discordJoinBtn
        discordJoinStroke.Color = Config.ThemeColor
        discordJoinStroke.Thickness = 1
        table.insert(allHubStrokes, discordJoinStroke)
        AddHoverGlow(discordJoinBtn, discordJoinStroke)
        ...
        discordJoinBtn.MouseButton1Click:Connect(function()
            ...
            if copied then
                ...
                ShowNotification("Discord: " .. discordUrl)
            end
        end)
        ...
        avtCard.BackgroundTransparency = 0.05
        avtCard.BorderSizePixel = 0
        avtCard.LayoutOrder = 3
        ...
        infoCard.BackgroundTransparency = 0.05
        ...
        -- AutomaticCanvasSize handles the Creative content height.
    else
...
local function SwitchTab(tabName)
    ...
    local homeBackdrop = contentContainer:FindFirstChild("HomeBackdrop")
    if homeBackdrop then
        homeBackdrop.Visible = (tabName == "Home")
    end
...
task.spawn(function()
    while greenDotKey and greenDotKey.Parent do
        TweenService:Create(greenDotKey, TweenInfo.new(0.6), {BackgroundTransparency = 0.05}):Play()
        task.wait(0.6)
