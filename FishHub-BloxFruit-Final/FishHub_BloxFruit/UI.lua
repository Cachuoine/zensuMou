...
openIcon.Text = "^"
openIcon.ZIndex = 902
 table.insert(allThemeTexts, openIcon)
...
local localTimeBtn = Instance.new("TextButton")
localTimeBtn.Name = "LocalTime"
localTimeBtn.Parent = main
localTimeBtn.Size = UDim2.new(0, 140, 0, 24)
localTimeBtn.Position = UDim2.new(0, 12, 1, -3)
localTimeBtn.AnchorPoint = Vector2.new(0, 1)
localTimeBtn.BackgroundColor3 = Color3.fromRGB(32, 34, 44)
localTimeBtn.BorderSizePixel = 0
localTimeBtn.AutoButtonColor = false
localTimeBtn.Font = Enum.Font.Code
localTimeBtn.TextSize = 10
localTimeBtn.TextColor3 = Config.ThemeColor
localTimeBtn.Text = "LOCAL 00:00:00"
Instance.new("UICorner", localTimeBtn).CornerRadius = UDim.new(0, 6)
local localTimeStroke = Instance.new("UIStroke")
localTimeStroke.Parent = localTimeBtn
localTimeStroke.Color = Config.ThemeColor
localTimeStroke.Thickness = 1
table.insert(allHubStrokes, localTimeStroke)
...
local function UpdateLocalTime()
    ...
    if ok and stamp then
        localTimeBtn.Text = "LOCAL  " .. stamp
    else
        localTimeBtn.Text = "LOCAL  --:--:--"
    end
end
task.spawn(function()
    while localTimeBtn and localTimeBtn.Parent do
        UpdateLocalTime()
        task.wait(0.5)
    end
end)
    ...
    local ok, err = pcall(function()
        ...
        local context = {
            ...
            IsRainbowEnabled = function()
                return Config.RainbowEnabled == true
            end,
            SetRainbowEnabled = function(value)
                Config.RainbowEnabled = value == true
            end
        }
...
-- Debug telemetry removed: FPS / PING / MEM / PLAYERS / TIME sidebar is intentionally disabled.
