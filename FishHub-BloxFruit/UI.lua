...
local Config = {
    ...
    ToggleKeyAliases = { [Enum.KeyCode.LeftControl] = true },
...
-- Debug HUD is fully removed. Hidden compatibility references are kept only for gear.lua API compatibility.
local debugSidebarFrame = Instance.new("Frame")
debugSidebarFrame.Name = "DebugSidebar"
debugSidebarFrame.Parent = gui
debugSidebarFrame.Visible = false
debugSidebarFrame.Size = UDim2.fromOffset(1, 1)
debugSidebarFrame.BackgroundTransparency = 1
local debugSidebarText = Instance.new("TextLabel")
debugSidebarText.Parent = debugSidebarFrame
debugSidebarText.Visible = false
...
keyStatusSidebarFrame.Visible = false
keyStatusSidebarFrame.Size = UDim2.fromOffset(1, 1)
keyStatusSidebarFrame.BackgroundTransparency = 1
local debugDividerBetween = Instance.new("Frame")
debugDividerBetween.Name = "DebugDividerBetween"
debugDividerBetween.Parent = gui
debugDividerBetween.Visible = false
debugDividerBetween.Size = UDim2.fromOffset(1, 1)
debugDividerBetween.BackgroundTransparency = 1
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if SettingsController and SettingsController.IsListeningKey and SettingsController.IsListeningKey() then return end
    local key=input.KeyCode
    if Config.ToggleKey and key==Config.ToggleKey then
        ToggleMain()
        return
    end
    if type(Config.ToggleKeyAliases)=="table" and Config.ToggleKeyAliases[key]==true then
        ToggleMain()
    end
end)
...
local worldClockBtn = Instance.new("TextButton")
worldClockBtn.Name = "WorldClock"
worldClockBtn.Parent = main
worldClockBtn.Size = UDim2.new(0, 152, 0, 24)
worldClockBtn.Position = UDim2.new(0, 12, 1, -3)
worldClockBtn.AnchorPoint = Vector2.new(0, 1)
worldClockBtn.BackgroundColor3 = Color3.fromRGB(25, 27, 38)
worldClockBtn.BorderSizePixel = 0
worldClockBtn.AutoButtonColor = false
worldClockBtn.Font = Enum.Font.Code
worldClockBtn.TextSize = 10
worldClockBtn.TextColor3 = Config.ThemeColor
worldClockBtn.Text = "LOCAL 00:00:00"
worldClockBtn.TextXAlignment = Enum.TextXAlignment.Center
Instance.new("UICorner", worldClockBtn).CornerRadius = UDim.new(0, 7)
local worldClockStroke = Instance.new("UIStroke")
worldClockStroke.Parent = worldClockBtn
worldClockStroke.Color = Config.ThemeColor
worldClockStroke.Thickness = 1
worldClockStroke.Transparency = 0.15
table.insert(allHubStrokes, worldClockStroke)
table.insert(allThemeTexts, worldClockBtn)

local function GetUtcTimestamp()
...
local function FormatClock(timestamp, offsetSeconds)
    local t = os.date("!*t", timestamp + offsetSeconds)
    return string.format("%02d:%02d:%02d", t.hour, t.min, t.sec)
end

-- The local clock uses the executor/Roblox local timezone when available.
local function GetLocalClock()
    local ok, value = pcall(function()
        return os.date("%H:%M:%S")
    end)
    return ok and value or FormatClock(GetUtcTimestamp(), 0)
end

local worldZones = {
    {name = "LOCAL", localTime = true},
    {name = "UTC", offset = 0},
    {name = "LONDON", offset = 0},
    {name = "NEW YORK", offset = -4 * 3600},
    {name = "LOS ANGELES", offset = -7 * 3600},
    {name = "PARIS", offset = 2 * 3600},
    {name = "DUBAI", offset = 4 * 3600},
    {name = "BANGKOK", offset = 7 * 3600},
    {name = "VIETNAM", offset = 7 * 3600},
    {name = "TOKYO", offset = 9 * 3600},
    {name = "SYDNEY", offset = 10 * 3600},
}

local clockIndex = 1
local clockHover = false
worldClockBtn.MouseEnter:Connect(function()
    clockHover = true
    TweenService:Create(worldClockBtn, TweenInfo.new(0.18, Enum.EasingStyle.Quint), {
        BackgroundColor3 = Color3.fromRGB(34, 37, 50)
    }):Play()
end)
worldClockBtn.MouseLeave:Connect(function()
    clockHover = false
    TweenService:Create(worldClockBtn, TweenInfo.new(0.18, Enum.EasingStyle.Quint), {
        BackgroundColor3 = Color3.fromRGB(25, 27, 38)
    }):Play()
end)
worldClockBtn.MouseButton1Click:Connect(function()
    clockIndex = (clockIndex % #worldZones) + 1
end)

task.spawn(function()
    while worldClockBtn and worldClockBtn.Parent do
        local zone = worldZones[clockIndex]
        local text
        if zone.localTime then
            text = "LOCAL  " .. GetLocalClock()
        else
            text = zone.name .. "  " .. FormatClock(GetUtcTimestamp(), zone.offset)
        end
        worldClockBtn.Text = text
        worldClockBtn.TextColor3 = Config.ThemeColor
        task.wait(0.25)
    end
end)
...
-- Function modules are loaded into an inner host inside FunctionTab.
local FunctionUrls = {
    ...
    ItemQuest = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub-BloxFruit/s1/item.lua",
...
local loadingFunctionModule = false
local activeFunctionModule = nil
local functionModuleGeneration = 0
local LoadTabUrl

local function ClearFunctionTab()
    local frame = tabs.Function
...
local function CreateModuleShell(moduleName)
    local frame = tabs.Function
    ...
    header.BackgroundColor3 = Color3.fromRGB(8, 9, 14)
    header.BorderSizePixel = 0
    Instance.new("UICorner", header).CornerRadius = UDim.new(0, 12)

    local headerStroke = Instance.new("UIStroke")
    headerStroke.Parent = header
    headerStroke.Color = Config.ThemeColor
    headerStroke.Thickness = 1
    headerStroke.Transparency = 0.2
    table.insert(allHubStrokes, headerStroke)

    local back = Instance.new("TextButton")
    back.Name = "Back"
    back.Parent = header
    back.Size = UDim2.fromOffset(34, 30)
    back.Position = UDim2.fromOffset(7, 7)
    back.BackgroundColor3 = Color3.fromRGB(20, 22, 31)
    back.BorderSizePixel = 0
    back.AutoButtonColor = false
    back.Text = "‹"
    back.Font = Enum.Font.GothamBlack
    back.TextSize = 22
    back.TextColor3 = Config.ThemeColor
    ...
    table.insert(allThemeTexts, back)

    local search = Instance.new("TextBox")
    search.Name = "Search"
    search.Parent = header
    search.Size = UDim2.new(1, -150, 0, 30)
    search.Position = UDim2.fromOffset(48, 7)
    search.BackgroundColor3 = Color3.fromRGB(20, 22, 31)
    ...
    search.PlaceholderColor3 = Color3.fromRGB(120, 125, 140)
    search.Text = ""
    ...
    searchIcon.Size = UDim2.fromOffset(24, 30)
    searchIcon.Position = UDim2.new(1, -36, 0, 7)
    searchIcon.BackgroundTransparency = 1
    searchIcon.Font = Enum.Font.GothamBold
    searchIcon.Text = "⌕"
    searchIcon.TextSize = 20
    searchIcon.TextColor3 = Config.ThemeColor
    searchIcon.ZIndex = 3
    table.insert(allThemeTexts, searchIcon)

    local title = Instance.new("TextLabel")
    title.Parent = root
    title.Size = UDim2.new(1, -10, 0, 24)
    title.Position = UDim2.fromOffset(5, 52)
    title.BackgroundTransparency = 1
    title.Font = Enum.Font.GothamBold
    title.TextSize = 10
    title.TextColor3 = Color3.fromRGB(155, 160, 175)
    title.Text = string.upper(moduleName) .. "  •  MODULE"
    title.TextXAlignment = Enum.TextXAlignment.Left

    local host = Instance.new("ScrollingFrame")
    host.Name = "ModuleContent"
    host.Parent = root
    host.Position = UDim2.fromOffset(0, 80)
    host.Size = UDim2.new(1, 0, 0, 0)
    host.AutomaticCanvasSize = Enum.AutomaticSize.Y
    host.CanvasSize = UDim2.new(0, 0, 0, 0)
    host.BackgroundTransparency = 1
    host.BorderSizePixel = 0
    host.ScrollBarThickness = 0
    host.ScrollBarImageTransparency = 1
    host.AutomaticSize = Enum.AutomaticSize.Y

    local function ApplyModuleFilter(query)
        query = string.lower(tostring(query or "")):gsub("^%s+", ""):gsub("%s+$", "")
        for _, obj in ipairs(host:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                if obj:GetAttribute("FishHubSearchIgnore") then
                    continue
                end
                local haystack = string.lower(tostring(obj.Text or "") .. " " .. tostring(obj.Name or ""))
                if query == "" then
                    obj.Visible = true
                else
                    obj.Visible = string.find(haystack, query, 1, true) ~= nil
                end
            end
        end
    end
    ...
    search:GetPropertyChangedSignal("Text"):Connect(function()
        ApplyModuleFilter(search.Text)
    end)

    back.MouseEnter:Connect(function()
        TweenService:Create(back, TweenInfo.new(0.16, Enum.EasingStyle.Quint), {
            BackgroundColor3 = Config.ThemeColor,
            TextColor3 = Color3.fromRGB(20, 22, 30)
        }):Play()
    end)
    ...
    back.Activated:Connect(function()
        functionModuleGeneration += 1
        activeFunctionModule = nil
        loadedTabs.Function = false
        loadingFunctionModule = false
        task.spawn(function()
            LoadTabUrl("Function")
        end)
    end)

    -- Keep module shell accents synchronized with theme/rainbow.
    task.spawn(function()
        while root.Parent do
            local accent = Config.ThemeColor
            headerStroke.Color = accent
            backStroke.Color = accent
            back.TextColor3 = accent
...
local function LoadFunctionModule(moduleKey)
    local url = FunctionUrls[moduleKey]
    if type(url) ~= "string" or url == "" or loadingFunctionModule then
        return false
    end

    loadingFunctionModule = true
    functionModuleGeneration += 1
    local generation = functionModuleGeneration
    activeFunctionModule = moduleKey

    local host = CreateModuleShell(moduleKey)
    if not host then
        loadingFunctionModule = false
        return false
    end
    ...
    local ok, err = pcall(function()
        ...
        if type(source) ~= "string" or #source < 10 then
            error("Remote module returned empty/invalid source")
        end
    ...
    loadingFunctionModule = false
    if generation ~= functionModuleGeneration then
        return false
    end

    if not ok then
        warn("[FishHub] " .. moduleKey .. " failed to load: " .. tostring(err))
        ShowNotification(moduleKey .. " failed to load: " .. tostring(err))
        return false
    end
...
LoadTabUrl = function(tabName)
    ...
    local ok, err = pcall(function()
        ...
        local context = {
            ...
            ShowNotification = ShowNotification,
            TabName = tabName,
            LoadFunction = LoadFunctionModule,
            Navigate = LoadFunctionModule
        }
...
-- Debug HUD/FPS/Ping/Mem/Players/Time telemetry intentionally removed.
