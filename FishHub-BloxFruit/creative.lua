...
-- Avatar bên trái giữ nguyên logic cache
local AV_CENTER=UDim2.fromOffset(62,62)
local avRing=N('Frame',{Parent=header,AnchorPoint=Vector2.new(.5,.5),Position=AV_CENTER,Size=UDim2.fromOffset(102,102),BackgroundTransparency=1});Circle(avRing)
Stroke(avRing,2.5,.15,accent())

local avHolder=N('Frame',{Parent=header,AnchorPoint=Vector2.new(.5,.5),Position=AV_CENTER,Size=UDim2.new(0,86,0,86),BackgroundColor3=Color3.fromRGB(6,7,11),BorderSizePixel=0});Circle(avHolder)
Stroke(avHolder,1.4,.25)
local av=N('ImageLabel',{Parent=avHolder,AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Image='',ScaleType=Enum.ScaleType.Crop});Circle(av)
local fallback=N('TextLabel',{Parent=avHolder,Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text='R',Font=Enum.Font.GothamBlack,TextSize=28,TextColor3=accent(),ZIndex=3})
...
N('TextLabel',{Parent=header,Position=UDim2.fromOffset(TEXT_X,78),Size=UDim2.new(1,-(TEXT_X+16),0,50),BackgroundTransparency=1,Text='FishHub interface — social links.\nTap a card below to copy the link.',Font=Enum.Font.GothamMedium,TextSize=9.5,TextColor3=Color3.fromRGB(148,153,168),TextWrapped=true,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top})
...
local titleBar=N('Frame',{Parent=titleWrap,Position=UDim2.fromOffset(0,3),Size=UDim2.fromOffset(3,26),BackgroundColor3=accent(),BorderSizePixel=0});Round(titleBar,2)
...
N('TextLabel',{Parent=titleWrap,Position=UDim2.fromOffset(14,17),Size=UDim2.new(1,-14,0,14),BackgroundTransparency=1,Text='Click a card to copy its link',Font=Enum.Font.GothamMedium,TextSize=8.5,TextColor3=Color3.fromRGB(130,135,150),TextXAlignment=Enum.TextXAlignment.Left})

------------------------------------------------------------
-- SOCIAL CARDS (Màu chuẩn thực tế, không dùng ảnh rbxassetid)
------------------------------------------------------------
...
local socials={
	{name='DISCORD', symbol='DC', url='https://discord.gg/zFN6Nd99fC', brand=Color3.fromRGB(88,101,242), glow=Color3.fromRGB(114,137,218)},
	{name='FACEBOOK', symbol='FB', url='https://www.facebook.com/dao.huy.lam.09/', brand=Color3.fromRGB(24,119,242), glow=Color3.fromRGB(66,140,244)},
	{name='TIKTOK', symbol='TT', url='https://www.tiktok.com/@daolam.trh', brand=Color3.fromRGB(22,24,35), glow=Color3.fromRGB(56,222,222)},
}
...
for i,d in ipairs(socials)do
	...
	-- Khung chứa icon chữ thương hiệu thay vì dùng ảnh load lỗi
	local iconCenter=UDim2.fromOffset(46,48)
	local iconHolder=N('Frame',{Parent=card,AnchorPoint=Vector2.new(.5,.5),Position=iconCenter,Size=UDim2.fromOffset(60,60),BackgroundColor3=d.brand,BorderSizePixel=0});Circle(iconHolder)
	local innerStroke=Stroke(iconHolder,1.5,.3,d.glow)
	
	-- Chữ biểu tượng đại diện cực kỳ sắc nét và sang trọng
	N('TextLabel',{Parent=iconHolder,Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text=d.symbol,Font=Enum.Font.GothamBlack,TextSize=18,TextColor3=Color3.fromRGB(255,255,255),TextXAlignment=Enum.TextXAlignment.Center,TextYAlignment=Enum.TextYAlignment.Center})
	...
	local btn=N('TextButton',{Parent=card,Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,AutoButtonColor=false,Text=''})

	btn.MouseEnter:Connect(function()
		Tween(sc,.18,{Scale=1.05},Enum.EasingStyle.Back)
		Tween(cardStroke,.18,{Transparency=.15,Color=d.glow})
		Tween(card,.18,{BackgroundColor3=Color3.fromRGB(15,17,27)})
		Tween(innerStroke,.18,{Transparency=0,Thickness=2})
	end)
	btn.MouseLeave:Connect(function()
		Tween(sc,.18,{Scale=1})
		Tween(cardStroke,.18,{Transparency=.55,Color=Color3.fromRGB(255,255,255)})
		Tween(card,.18,{BackgroundColor3=PANEL_BG})
		Tween(innerStroke,.18,{Transparency=.3,Thickness=1.5})
	end)
	btn.Activated:Connect(function()
		pcall(function()if setclipboard then setclipboard(d.url)end end)
		notify('Copied '..d.name..' link!')
		Tween(innerStroke,.15,{Color=SUCCESS,Transparency=0,Thickness=2.5})
		Tween(sc,.15,{Scale=1.12},Enum.EasingStyle.Back)
		task.delay(.55,function()
			if innerStroke and innerStroke.Parent then
				Tween(innerStroke,.35,{Color=d.glow,Transparency=.3,Thickness=1.5})
			end
