...
local Config = {
    ...
    -- Rainbow is supported by the accent system. Keep it off until enabled
    -- by the Settings/Gear controller so normal Theme Color remains intact.
...
local accentLineGlow = Instance.new("UIStroke")
accentLineGlow.Parent = line
accentLineGlow.Thickness = 4
accentLineGlow.Transparency = 0.78
accentLineGlow.Color = Config.ThemeColor

local accentLineGradient = Instance.new("UIGradient")
accentLineGradient.Parent = line
accentLineGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(20, 22, 30)),
    ColorSequenceKeypoint.new(0.5, Config.ThemeColor),
...
task.spawn(function()
    ...
    while line and line.Parent do
        ...
        if Config.RainbowEnabled == true then
            local speedPercent = math.clamp(tonumber(Config.RainbowSpeedPercent) or 100, 1, 300)
            accent = Color3.fromHSV((os.clock() * 0.055 * (speedPercent / 100)) % 1, 0.88, 1)
        else
            accent = GetCurrentAccentColor and GetCurrentAccentColor() or Config.ThemeColor
        end
        line.BackgroundColor3 = accent
        accentLineGlow.Color = accent
        accentLineGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, accent:Lerp(Color3.fromRGB(18, 20, 28), 0.92)),
            ColorSequenceKeypoint.new(0.5, accent),
            ColorSequenceKeypoint.new(1, accent:Lerp(Color3.fromRGB(18, 20, 28), 0.92)),
        })
        offset += 0.018
        if offset > 1 then offset = -1 end
        accentLineGradient.Offset = Vector2.new(offset, 0)
        task.wait(Config.RainbowEnabled == true and 0.035 or 0.12)
    end
end)
...
-- ============================================================
-- Premium Circle Controls / Theme + Rainbow Accent System
-- ============================================================
local circleButtons = {}
local circleRainbowObjects = {}

local function GetCircleAccent()
    local rainbowEnabled = Config.RainbowEnabled == true
    if rainbowEnabled then
        local speedPercent = math.clamp(tonumber(Config.RainbowSpeedPercent) or 100, 1, 300)
        local hue = (os.clock() * 0.055 * (speedPercent / 100)) % 1
        return Color3.fromHSV(hue, 0.88, 1)
    end

    return GetCurrentAccentColor and GetCurrentAccentColor() or Config.ThemeColor
end

local function ApplyCircleAccent(btn, color)
    if not btn or not btn.Parent or not color then return end

    local stroke = btn:FindFirstChild("CircleStroke")
    local glow = btn:FindFirstChild("CircleGlow")
    local gradient = btn:FindFirstChild("CircleGradient")
    local inner = btn:FindFirstChild("CircleInner")

    local darkColor = color:Lerp(Color3.fromRGB(18, 20, 29), 0.72)
    local midColor = color:Lerp(Color3.fromRGB(35, 38, 52), 0.55)
    local brightColor = color:Lerp(Color3.new(1, 1, 1), 0.18)
    ...
    if inner then
        ...
        inner.BackgroundTransparency = 0.91
    end
    if gradient then
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, midColor),
            ColorSequenceKeypoint.new(0.48, darkColor),
...
local function CreateCircleButton(text, xOffset)
    local btn = Instance.new("TextButton")
    btn.Parent = main
    btn.Name = "CircleControl_" .. tostring(text)
    btn.Size = UDim2.new(0, 29, 0, 29)
    ...
    btn.Font = Enum.Font.GothamBold
    btn.TextSize = 13
    btn.TextColor3 = Color3.fromRGB(242, 244, 250)
    btn.ZIndex = 12
    ...
    local gradient = Instance.new("UIGradient")
    gradient.Name = "CircleGradient"
    gradient.Parent = btn
    gradient.Rotation = 135

    local btnStroke = Instance.new("UIStroke")
    btnStroke.Name = "CircleStroke"
    btnStroke.Parent = btn
    btnStroke.Thickness = 1.35
    btnStroke.Transparency = 0.04
    table.insert(allHubStrokes, btnStroke)
    ...
    inner.BackgroundTransparency = 0.92
    ...
    local btnScale = Instance.new("UIScale")
    btnScale.Name = "CircleScale"
    btnScale.Parent = btn
    btnScale.Scale = 1

    table.insert(circleButtons, btn)
    table.insert(circleRainbowObjects, {
        button = btn,
        stroke = btnStroke,
        glow = glow,
        inner = inner,
    ...
    ApplyCircleAccent(btn, GetCircleAccent())
    AddHoverGlow(btn, btnStroke)

    btn.MouseEnter:Connect(function()
        local accent = GetCircleAccent()
        ApplyCircleAccent(btn, accent)
        innerStroke.Color = accent
        TweenService:Create(btnScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Scale = 1.13
        }):Play()
        TweenService:Create(glow, TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Thickness = 8,
            Transparency = 0.48
        }):Play()
        TweenService:Create(inner, TweenInfo.new(0.18, Enum.EasingStyle.Quint), {
            BackgroundTransparency = 0.82
        }):Play()
    end)

    btn.MouseLeave:Connect(function()
        TweenService:Create(btnScale, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Scale = 1
        }):Play()
        TweenService:Create(glow, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Thickness = 5,
        ...
        TweenService:Create(inner, TweenInfo.new(0.22, Enum.EasingStyle.Quint), {
            BackgroundTransparency = 0.92
        }):Play()
    end)

    btn.MouseButton1Down:Connect(function()
        TweenService:Create(btnScale, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Scale = 0.91
        }):Play()
    end)

    btn.MouseButton1Up:Connect(function()
        TweenService:Create(btnScale, TweenInfo.new(0.16, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Scale = 1.13
        }):Play()
    end)
...
-- Continuously keeps the circle controls synchronized with Theme Color
-- or Rainbow mode, including changes made later by the Settings controller.
task.spawn(function()
    while gui and gui.Parent do
        local accent = GetCircleAccent()
        for _, item in ipairs(circleRainbowObjects) do
            if item.button and item.button.Parent then
                ApplyCircleAccent(item.button, accent)
                if item.innerStroke then
                    item.innerStroke.Color = accent
                end
            end
        end
        task.wait(Config.RainbowEnabled == true and 0.035 or 0.12)
