...
-- =========================================================================
-- cleanup cũ
if player.PlayerGui:FindFirstChild("FishHubShop") then
    player.PlayerGui.FishHubShop:Destroy()
end
...
-- icon search (gradient stroke + glyph)
...
-- Vertical divider (kẻ dọc ngăn cách)
...
local activeIndex = 1
local tabButtons  = {}
local tabIndicator -- kẻ dọc di chuyển

-- vẽ trước kẻ dọc indicator
tabIndicator = New("Frame", {
    Parent = leftCol,
    AnchorPoint = Vector2.new(0, 0.5),
    Position = UDim2.new(0, 0, 0, 22),
    Size = UDim2.new(0, 3, 0, 0), -- sẽ được tween
    BackgroundColor3 = CONFIG.ThemeColor,
    BorderSizePixel = 0,
    ZIndex = 10,
})
...
for i, info in ipairs(TAB_LIST) do
    ...
    -- glow accent bên trái (kẻ dọc nhỏ) — ẩn mặc định
...
-- rainbow support (optional, không bật mặc định)
