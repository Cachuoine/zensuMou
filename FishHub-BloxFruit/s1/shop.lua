...
local TABS = {
    {
        key     = "sword",
        title   = "Sword Shop",
        subtitle= "Legendary blades & cutlasses",
        icon    = "🗡",
    ...
    },
    {
        key     = "gun",
        title   = "Gun Shop",
        subtitle= "Rifles, pistols & snipers",
        icon    = "🔫",
    ...
    },
    {
        key     = "melee",
        title   = "Fighting Shop",
        subtitle= "Fists, claws & combat arts",
        icon    = "🥊",
    ...
    },
    {
        key     = "miss",
        title   = "Misc Shop",
        subtitle= "Accessories & consumables",
        icon    = "✨",
...
local function Gradient(parent, c1, c2, rotation)
    local g = New("UIGradient", { Parent = parent, Color = ColorSequence.new(c1, c2), Rotation = rotation or 90 })
    return g
end
...
local searchInput = New("TextBox", {
    ...
    PlaceholderText = "Search in shop...",
...
-- ============================================================
-- MAIN AREA (LEFT TABS + DIVIDER + RIGHT CONTENT)
-- ============================================================
...
local dividerGrad = Gradient(divider, Color3.fromRGB(60, 65, 85), Color3.fromRGB(20, 22, 32), 90)
...
local rTitle = New("TextLabel", { Parent = rHeader, Position = UDim2.new(0, 58, 0, 4), Size = UDim2.new(1, -120, 0, 24), BackgroundTransparency = 1, Font = Enum.Font.GothamBold, TextSize = 16, TextColor3 = C.textMain, TextXAlignment = Enum.TextXAlignment.Left, Text = "Shop" })
...
-- Right body (scrolling content from URL)
...
-- ============================================================
local tabButtons   = {}
local tabAccents   = {} -- vertical accent bars
local activeIndex  = 1
...
local function buildTabCard(data, order)
    ...
    Corner(vLine, 2)
    local vLineGrad = Gradient(vLine, data.accent, Color3.fromRGB(255, 255, 255), 90)
    vLine.BackgroundTransparency = 1
    vLine.Size = UDim2.new(0, 3, 0, 0)

    -- Glow background
    local glow = New("Frame", { Parent = card, Size = UDim2.fromScale(1, 1), BackgroundColor3 = data.accent, BackgroundTransparency = 1, BorderSizePixel = 0 })
    Corner(glow, 10)
    ...
    return card, vLine, iconBubble, iconLabel, title, sub, glow
end

for i, data in ipairs(TABS) do
    local card, vLine, iconBubble, iconLabel, title, sub, glow = buildTabCard(data, i)
    tabButtons[i] = { card = card, vLine = vLine, data = data, iconBubble = iconBubble, iconLabel = iconLabel, title = title, sub = sub, glow = glow }
end

-- ============================================================
-- TAB SWITCH + ANIMATION (vertical accent line)
-- ============================================================
...
local function setActive(idx)
    ...
    -- Vertical accent line slides between tabs
    -- (each tab card has its own vLine; we hide the previous, animate the new)
    if prev then
    ...
    -- Subtle slide-in for right content
...
connection = RunService.RenderStepped:Connect(function()
    ...
    for _, b in ipairs({"UIStroke"}) do end
    -- strokes on root panels follow their tab accent
    for i, t in ipairs(tabButtons) do
