...
-- Split Container (Left Menu + Right Content)
...
local menuLayout = New("UIListLayout", { Parent = menuContainer, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 6) })
...
local function loadUrlContent(url)
    ...
    if contentCache[url] then
        local label = New("TextLabel", { Parent = contentScroll, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1, Text = contentCache[url], Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = Color3.fromRGB(210, 215, 230), TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true })
        return
    end
...
searchInput:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, child in ipairs(contentScroll:GetChildren()) do
        if child:IsA("TextLabel") and child.Text ~= "Loading content..." and not child.Text:find("Failed") then
            -- Simple text filtering if needed, or leave full display
        end
    end
end)
...
connection = RunService.RenderStepped:Connect(function()
    ...
    if activeTabButton then
        local ind = activeTabButton:FindFirstChildOfClass("Frame")
        if ind then ind.BackgroundColor3 = a end
        local lbl = activeTabButton:FindFirstChildOfClass("TextLabel")
        if lbl and activeTabButton == activeTabButton then -- active check
            -- Keep active label colored
        end
    end
end)
