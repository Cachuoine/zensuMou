...
local function GetRemoteRenderer(url, cacheKey)
    if RemoteCache[cacheKey] then
        return RemoteCache[cacheKey]
    end

    if not url or url == "" or url:find("YOUR_USERNAME", 1, true) then
        warn("[FishHub] Missing GitHub raw URL for " .. cacheKey)
        return nil
    end

    if RemoteLoading[cacheKey] then
        return nil
    end
    RemoteLoading[cacheKey] = true
    ...
    local ok, result = pcall(function()
        ...
        local renderer = chunk()
        if type(renderer) ~= "function" then
            error("Remote module must return a renderer function")
        end
        return renderer
    end)

    RemoteLoading[cacheKey] = nil

    if not ok then
        warn("[FishHub] Failed to load " .. cacheKey .. ": " .. tostring(result))
        ShowNotification("Failed to load " .. cacheKey .. " module")
        return nil
    end
...
local function BuildRemoteContext()
    return {
        Config = Config,
...
local function CreateTabContent(name)
    ...
    if name == "Home" then
        local renderer = GetRemoteRenderer(HOME_URL, "Home")
        if renderer then
            local ok, err = pcall(renderer, BuildRemoteContext(), tabFrame)
            if not ok then
                warn("[FishHub] Home renderer error: " .. tostring(err))
                ShowNotification("Home module error")
            end
        end
    elseif name == "Function" then
        local renderer = GetRemoteRenderer(FUNCTION_URL, "Function")
        if renderer then
            local ok, err = pcall(renderer, BuildRemoteContext(), tabFrame)
            if not ok then
                warn("[FishHub] Function renderer error: " .. tostring(err))
                ShowNotification("Function module error")
            end
        end
