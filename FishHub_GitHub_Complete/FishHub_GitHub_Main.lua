...
local function CreateTabContent(name)
    ...
    elseif name == "Function" then
        ...
        getgenv().FishHubFeatureAPI = FeatureAPI
        ...
        local function LoadFeatureScript(featureName)
            ...
            if not feature then
                ShowNotification("Unknown feature: " .. tostring(featureName))
                return false
            end

            local url = TrimString(feature.URL)
            if url == "" then
                ShowNotification(featureName .. " URL is not set yet.")
                return false
            end

            if LoadingFeatureModules[featureName] then
                ShowNotification(featureName .. " is already loading...")
                return false
            end

            if type(loadstring) ~= "function" then
                ShowNotification("loadstring is unavailable in this environment.")
                return false
            end
            ...
            if type(cached) == "function" then
                ...
                if not ok then
                    ShowNotification(featureName .. " failed: " .. tostring(err))
                    return false
                end
                ShowNotification(featureName .. " restarted.")
                return true
            ...
            ShowNotification("Loading " .. featureName .. "...")
            ...
            if not ok then
                ShowNotification(featureName .. " load failed: " .. tostring(result))
                return false
            end
            ...
            if type(result) == "function" then
                ...
                if not runOK then
                    ShowNotification(featureName .. " failed: " .. tostring(runError))
                    return false
                end
            ...
            ShowNotification(featureName .. " loaded successfully.")
            return true
        end
        ...
        local function CreateCategoryCard(title, icon, desc, order)
            ...
            card.MouseButton1Click:Connect(function()
                ...
                if url == "" then
                    ShowNotification(title .. " is not configured: add its GitHub RAW URL first.")
                    return
                end

                local loaded = LoadFeatureScript(title)
                if not loaded then
                    -- Failed remote modules must not replace the Function page.
                    return
                end
