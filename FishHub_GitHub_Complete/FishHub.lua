...
bottomDock.Parent = main
bottomDock.Size = UDim2.new(1, -20, 0, 52)
bottomDock.Position = UDim2.new(0, 10, 1, -58)
bottomDock.BackgroundTransparency = 1
bottomDock.ZIndex = 20

local joinDiscordBtn = Instance.new("TextButton")
joinDiscordBtn.Parent = bottomDock
joinDiscordBtn.Size = UDim2.new(0, 140, 0, 26)
joinDiscordBtn.Position = UDim2.new(0, 0, 1, -26)
...
joinDiscordBtn.ZIndex = 21
Instance.new("UICorner", joinDiscordBtn).CornerRadius = UDim.new(0, 6)

local rightBottomStatus = Instance.new("TextLabel")
rightBottomStatus.Parent = bottomDock
rightBottomStatus.Size = UDim2.new(1, -155, 0, 26)
rightBottomStatus.Position = UDim2.new(1, 0, 1, -26)
rightBottomStatus.AnchorPoint = Vector2.new(1, 0)
...
rightBottomStatus.TextXAlignment = Enum.TextXAlignment.Center
...
-- ==================== EXTERNAL FUNCTION MODULE SYSTEM ====================
-- FishHub remains the UI shell. Each function page is supplied by its own module.
-- Set MODULE_BASE_URL to the raw GitHub folder containing the 8 modules.
local MODULE_BASE_URL = "https://raw.githubusercontent.com/Cachuoine/zensuMou/refs/heads/main/FishHub_GitHub_Complete/FunctionModules"
local MODULE_FILES = {
    ["Shop"] = "Shop.lua",
    ["Setting farm"] = "SettingFarm.lua",
    ["Farm"] = "Farm.lua",
    ["Item and Quest"] = "ItemAndQuest.lua",
    ["Teleport Island"] = "TeleportIsland.lua",
    ["Fruit"] = "Fruit.lua",
    ["ESP"] = "ESP.lua",
    ["Setting"] = "Setting.lua",
}
...
local function LoadFunctionModule(title)
    local fileName = MODULE_FILES[title]
    if not fileName then return nil, "No module configured for " .. tostring(title) end
    if MODULE_CACHE[title] then return MODULE_CACHE[title] end
    local url = MODULE_BASE_URL .. "/" .. fileName
    local ok, source = pcall(function() return game:HttpGet(url) end)
    if not ok or type(source) ~= "string" then
        return nil, "HTTP failed: " .. tostring(fileName)
    end
    local compileOk, moduleOrErr = pcall(function()
        return loadstring(source, "@FishHub/" .. fileName)()
    end)
    if not compileOk or type(moduleOrErr) ~= "table" then
        return nil, "Module load failed: " .. tostring(moduleOrErr)
    end
    MODULE_CACHE[title] = moduleOrErr
    return moduleOrErr
end

local function RenderFunctionModule(title, contentLabel)
    contentLabel.Text = "Loading " .. title .. " module..."
    task.spawn(function()
        local module, err = LoadFunctionModule(title)
        if not module then
            contentLabel.Text = "MODULE ERROR\n\n" .. tostring(err)
            return
        end
        local ok, result = pcall(function()
            if type(module.Render) == "function" then
            ...
            return module.Description or "Module loaded successfully."
        end)
        if not ok then
            contentLabel.Text = "MODULE ERROR\n\n" .. tostring(result)
        elseif type(result) == "string" and contentLabel.Text == "Loading " .. title .. " module..." then
...
local function CreateTabContent(name)
    ...
    -- Reserve a clean bottom area so the fixed Discord/status dock never
    -- covers the last card while the user scrolls.
    local tabBottomPadding = Instance.new("UIPadding")
    tabBottomPadding.Parent = tabFrame
    tabBottomPadding.PaddingBottom = UDim.new(0, 62)
    ...
    if name == "Home" then
        ...
        local function CreateSectionDivider(text, order)
            ...
            dividerContainer.Size = UDim2.new(1, 0, 0, 38)
            ...
            local function makeFadeLine(parent, side, y)
                local line = Instance.new("Frame")
                line.Parent = parent
                line.BackgroundColor3 = Config.ThemeColor
                line.BorderSizePixel = 0
                line.Size = UDim2.new(0.5, -lineGap, 0, lineThickness)
                line.Position = side == "left"
                    and UDim2.new(0, 0, 0, y)
                    or UDim2.new(0.5, lineGap, 0, y)
                line.ZIndex = 1
                ...
                if side == "left" then
                    gradient.Color = ColorSequence.new({
                        ...
                        ColorSequenceKeypoint.new(0.55, Color3.new(0.72, 0.72, 0.72)),
                        ColorSequenceKeypoint.new(1, Color3.new(0.05, 0.05, 0.05))
                    })
                else
                    gradient.Color = ColorSequence.new({
                        ColorSequenceKeypoint.new(0, Color3.new(0.05, 0.05, 0.05)),
                        ColorSequenceKeypoint.new(0.45, Color3.new(0.72, 0.72, 0.72)),
            ...
            makeFadeLine(dividerContainer, "left", centerY)
            makeFadeLine(dividerContainer, "right", centerY)

            local textLbl = Instance.new("TextLabel")
            textLbl.Parent = dividerContainer
            textLbl.Size = UDim2.new(0, 0, 0, 18)
            textLbl.Position = UDim2.new(0.5, 0, 0, centerY - 9)
            ...
            -- A second divider sits directly below the title with no visual gap.
            makeFadeLine(dividerContainer, "left", centerY + 12)
            makeFadeLine(dividerContainer, "right", centerY + 12)
        end
    ...
    elseif name == "Function" then
        ...
        local function CreateCategoryCard(title, icon, desc, order)
            ...
            card.MouseButton1Click:Connect(function()
                ...
                -- Function content is isolated in an external module.
                RenderFunctionModule(title, subPageContentText)
